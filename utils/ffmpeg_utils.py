...
async def extract_last_frame(video_path: Union[str, Path], output_path: Union[str, Path]) -> bool:
    ...
    try:
        ...
        if result_duration.returncode != 0:
            logger.error(f"Error getting video duration: {result_duration.stderr.decode()}")
...
# Additional utility function to validate ffmpeg availability with installation option
async def ensure_ffmpeg() -> bool:
    """
    Ensure ffmpeg is available, install if necessary.
    
    Returns:
        bool: True if ffmpeg is available (either already present or successfully installed)
    """
    if check_ffmpeg():
