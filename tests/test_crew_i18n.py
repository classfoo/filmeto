#!/usr/bin/env python3
"""
Test script to verify crew member internationalization functionality
"""
import sys
...
def test_crew_i18n():
    """Test that crew members are loaded in the correct language"""
    print("Testing crew member internationalization...")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        crew_members_zh = crew_service.load_project_crew_members(project)
        ...
        if 'director' in crew_members_zh:
            director_desc = crew_members_zh['director'].config.description
            print(f"   Director description in Chinese: {director_desc}")
        ...
        crew_members_en = crew_service.refresh_project_crew_members(project)
        ...
        if 'director' in crew_members_en:
            director_desc = crew_members_en['director'].config.description
            print(f"   Director description in English: {director_desc}")
        ...
        translation_manager.switch_language("zh_CN")
        crew_members_zh2 = crew_service.refresh_project_crew_members(project)
        
        director_zh2 = crew_members_zh2.get('director')
        if director_zh2:
            director_desc2 = director_zh2.config.description
