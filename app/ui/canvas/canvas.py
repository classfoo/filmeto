...
class CanvasWidget(BaseTaskWidget):
    ...
    def __init__(self, workspace: Workspace):
        ...
        self.current_mode = DrawingMode.BRUSH
    ...
    def set_mode(self, mode):
        """Set the current drawing mode and update cursor for all layer widgets"""
        self.current_mode = mode
        
        # Update mode for all layer widgets
        for layer_widget in self.layer_widgets.values():
            layer_widget.set_mode(mode)
    ...
    def draw_shape(self):
        """Complete the shape drawing by rendering it to the active layer"""
        if self.current_mode == DrawingMode.SHAPE and self.active_layer_id and self.active_layer_id in self.layer_widgets:
            layer_widget = self.layer_widgets[self.active_layer_id]
            layer_widget.draw_shape(self.shape_start_pos, self.shape_end_pos, 
                                  self.brush_color, self.brush_size, self.brush_type,
                                  self.scale_factor, self.canvas_offset)
    
    def clear_canvas(self):
        """Clear the entire canvas (all layers)"""
        for layer_widget in self.layer_widgets.values():
            layer_widget.clear_draw()
        self.update()
