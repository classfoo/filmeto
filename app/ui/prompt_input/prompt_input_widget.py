...
class PromptInputWidget(BaseWidget):
    """
    ...
    def _setup_ui(self):
        ...
        self.text_edit.setMaximumHeight(40)  # Start in collapsed state
        self.text_edit.setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        self.text_edit.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.text_edit.installEventFilter(self)
        
        # Send button
        self.send_button = QPushButton("\ue846")  # Send icon from iconfont
        ...
        # Character counter
        self.char_counter = QLabel(tr("0 characters"))
        self.char_counter.setObjectName("prompt_char_counter")
        self.char_counter.setAlignment(Qt.AlignRight | Qt.AlignBottom)
        ...
        main_layout.addWidget(self.char_counter, 0, Qt.AlignRight)
    ...
    def _expand_input(self):
        """Expand input field to multi-line"""
        if not self._is_expanded:
            self._is_expanded = True
            self.text_edit.setMaximumHeight(120)
            self._update_text_edit_style()
    
    def _collapse_input(self):
        """Collapse input field to single line"""
    ...
    def _update_text_edit_style(self):
        """Update text edit style based on state"""
        if self._has_focus or self._is_expanded:
            self.text_edit.setStyleSheet("""
                QTextEdit#prompt_text_edit {
                    background-color: #2b2d30;
                    border: 1px solid #4080ff;
    ...
    def _on_text_changed(self):
        ...
        char_count = len(text)
        self.char_counter.setText(tr("{count} characters").replace("{count}", str(char_count)))
