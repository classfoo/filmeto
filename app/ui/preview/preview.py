...
from PySide6.QtWidgets import (
    ...
    QComboBox, QGroupBox, QFrame, QDockWidget, QScrollArea,
    QSizePolicy, QGridLayout
)
from PySide6.QtCore import Qt, QUrl, Slot, QTimer, QSize, Signal, QPointF, QRect, QPoint
from PySide6.QtGui import QPixmap, QImageReader, QMovie, QKeyEvent, QImage
...
class MediaPreviewWidget(BaseTaskWidget):
    ...
    def __init__(self, workspace):
        ...
        main_layout.addLayout(control_layout)  # 控制布局（按钮+帧选择器）
    ...
    def _load_image(self, image_path):
        ...
        # Resize the image_label to match the image size while preserving aspect ratio
        self._resize_image_label_with_pixmap(pixmap)
    ...
    def _load_gif(self, gif_path):
        ...
        if movie.isValid():
            ...
            # Resize the image_label to match the GIF size while preserving aspect ratio
            self._resize_image_label_with_movie(movie)
    ...
    def _switch_to_video(self, video_path):
        ...
        if self.video_widget.isVisible() and self.media_player.source().isValid():
            self.media_player.stop()
            self.media_player.setSource(QUrl())  # 清除旧视频源
            if self.video_capture:
                self.video_capture.release()
                self.video_capture = None
        ...
        self.is_playing = False
        self.play_pause_btn.setText("▶")
        self.play_pause_btn.setEnabled(False)  # 等待媒体加载完成后启用
    ...
    def resizeEvent(self, event):
        ...
        if self.image_label.pixmap() and not self.image_label.movie(): # 静态图片
            ...
            # For images, we should preserve original aspect ratio, but fit within the container
            # Calculate the maximum size that preserves aspect ratio within the container
            img_aspect = original_size.width() / original_size.height()
            container_aspect = container_size.width() / container_size.height()
            
            if img_aspect > container_aspect:
                # Image is wider than container (relative to height), scale by width
                final_width = min(container_size.width() - 20, original_size.width())  # -20 for margins
                final_height = int(final_width / img_aspect)
            else:
                # Image is taller than container (relative to width), scale by height
                final_height = min(container_size.height() - 20, original_size.height())  # -20 for margins
                final_width = int(final_height * img_aspect)
