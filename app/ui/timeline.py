...
from PySide6.QtWidgets import (
    ...
    QLabel, QVBoxLayout, QFrame, QSizePolicy, QGraphicsDropShadowEffect, QPushButton
)
...
class HorizontalTimeline(BaseTaskWidget):
    ...
    def add_new_card(self):
        ...
        try:
            ...
            # Use the timeline's method to add a new item
            new_index = timeline.add_item()
            
            # Create a new card widget and add it to the timeline before the add button
            # We need to remove the add button and stretch, add the new card, then re-add them
            self.timeline_layout.removeWidget(self.add_card_button)
            qt_utils.remove_last_stretch(self.timeline_layout)
            # Create the new card with the updated timeline data
